...
bool global_security::Init(){
    ...
    global_security::LOCAL_RSA_PUBLIC = pub_key_str;
    // 生成私钥
    ...
    global_security::LOCAL_RSA_PRIVATE = pri_key_str;
    // 获取服务器RSA公钥
    ...
    global_security::SERVER_RSA_PUBLIC = jsonObj.value("pubkey").toString();
    // 发送本地公钥以及特征密码给服务器
    ...
    QString dbpart256 = QString(QCryptographicHash::hash(strIpAddress.toUtf8(), QCryptographicHash::Sha256).toHex());
    // 将设备信息整合成字符串
    QString sysstr = hostName + deviceType + deviceName + strIpAddress + strMacAddr + "SSIMP" + QCoreApplication::applicationVersion() + dbpart256;
    // 计算syssstr的md5值
    ...
    // 计算特征值
    QString feature = QCryptographicHash::hash((dbpasswd + current_date_time.toString("yyyyMMddhhmmss") + random_str).toUtf8(), QCryptographicHash::Md5).toHex();
    return true;
}

QString global_security::getAesKey(){
    return global_security::AES_KEY;
}

QString global_security::getToken(){
    return global_security::TOKEN;
}
