...
TaskSetService::TaskSetService(QObject *parent) : QObject{parent} {}
  ...
  connect(net, &TcpNetUtils::requestFinished, this, [=, this]() {
    ...
    if (resp["code"] != "SE200") {
      ...
      for (int i = 0; i < data.size(); i++) {
        ...
        TaskSetData *taskset =
            new TaskSetData(tid, name, content, area, poslo, posli, cycle,
                            state, duration, this);
...
QQmlListProperty<TaskSetData>
TaskSetService::tasksets(QString keyname, float poslo, float posli) {
  // 生成临时列表
  // 判断是否为空
  if (m_tasksets->size() == 0) {
    getTaskSetList();
  ...
  for (int i = 0; i < m_tasksets->size(); i++) {
    if (m_tasksets->at(i)->name() != keyname) {
      if (m_tasksets->at(i)->poslo() > poslo - 0.002 &&
          m_tasksets->at(i)->poslo() < poslo + 0.002) {
        if (m_tasksets->at(i)->posli() > posli - 0.002 &&
            m_tasksets->at(i)->posli() < posli + 0.002) {
...
void TaskSetService::modifyTaskSet(QString tid, QString name, QString content,
                                   QString area, float poslo, float posli,
                                   int cycle, int state, int duration) {
  // 判断tid是否为0
  if (tid == "0") {
    // 添加任务
    ...
    // 更新任务
    updateTaskSet(tid, name, content, area, poslo, posli, cycle, state,
                  duration);
  }
}

void TaskSetService::convertFeedbackToTaskSet(QString taskname, QString content,
                                              QString area, int duration) {
  addTaskSet(taskname, content, area, 121.72439102324424, 41.61128739424099, -1,
             5, duration);
}
...
void TaskSetService::deleteTaskSet(QString tid) {
  ...
  connect(net, &TcpNetUtils::requestFinished, this, [=, this]() {
    ...
    if (resp["code"] != "SE200") {
      ...
      vctrler::showDialog(dialogType::DIALOG_TIP, dialogBtnType::DIALOG_OK,
                          "删除任务", "删除成功", "");
      emit taskSetListChanged(QQmlListProperty<TaskSetData>(this, m_tasksets));
    }
  });
...
void TaskSetService::addTaskSet(QString name, QString content, QString area,
                                float poslo, float posli, int cycle, int state,
                                int duration) {
...
void TaskSetService::updateTaskSet(QString tid, QString name, QString content,
                                   QString area, float poslo, float posli,
                                   int cycle, int state, int duration) {
...
void TaskSetService::fetchTaskSet(QString tid, QString name, QString content,
                                  QString area, float poslo, float posli,
                                  int cycle, int state, int duration) {
  for (int i = 0; i < m_tasksets->size(); i++) {
    if (m_tasksets->at(i)->tid() == tid.toInt()) {
